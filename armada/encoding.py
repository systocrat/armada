...
def packUTF8(data, endian=big_endian):
	ret = b''
	realData = data.encode('utf8')
	ret += packUShort(len(realData), endian)
	ret += realData
	return ret

def packArray(len_func, data_func, data, endian=big_endian):
	ret = b''
	ret += len_func(len(data), endian=endian)
	for i in xrange(len(data)):
		ret += data_func(data[i], endian=endian)
	return ret
...
def packPickle(obj, endian=big_endian):
	data = cPickle.dumps(obj)
	return packUInt(len(data), endian) + data
...
class Reader(object):
	...
	def readPickle(self):
		...
		if not self.has(length):
			raise ReadException()
		self.advance(length)
		return cPickle.loads(self.data[self.index - length:self.index])